...
const ProductPage = () => {
  ...
  return (
    <main className="flex-1 py-8 px-8 me-4 mt-8 text-gray-100 rounded-2xl shadow-xl min-h-[500px] max-h-[800px] overflow-hidden">
      <button
        onClick={() => router.back()}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 h-full mt-1">
        ...
        <div className="flex flex-col justify-between h-full overflow-hidden ml-4">
          <div className="space-y-8 overflow-y-auto pr-3 max-h-[620px] scrollbar-thin scrollbar-thumb-purple-600/60 scrollbar-track-transparent">
            ...
            <div className="flex items-center gap-3 mt-4">
              ...
              <span className="text-lg text-white/70">
                {product.stock > 0 ? `${product.stock} product` : '100+ sold'}
              </span>
            </div>
          ...
          >
            {isOutOfStock ? 'Princess, out of stock!' : 'Add to cart'}
          </button>
        </div>
        {/* Left: Product Image*/}
        <div className="flex flex-col items-center h-full overflow-hidden">
          <div className="flex justify-center items-center w-full h-[400px] bg-black/20 rounded-2xl shadow-inner p-4">
            <img
              src={previewImage}
              alt={product.name}
              className="rounded-xl max-h-full max-w-full object-contain transition duration-300"
            />
          </div>
          ...
          <div className="flex gap-2 mt-4 overflow-x-auto max-w-full pb-1">
            {product.images?.map((img, idx) => (
              <img
                ...
                className={`w-20 h-20 object-cover rounded-lg cursor-pointer transition-all duration-200 shadow-sm ${
                  img === previewImage
