...
const ProductPage = () => {
  ...
  return (
    <main className="flex-1 py-8 px-8 me-4 mt-20 text-gray-100 rounded-2xl shadow-xl min-h-screen max-h-[800px] overflow-hidden">
      <button
        onClick={() => router.back()}
        className="py-2 px-2 flex items-center gap-2 text-white hover:text-pink-400 transition"
      >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 h-full mt-1">
        ...
        <div className="flex flex-col justify-between h-full overflow-hidden">
          <div className="space-y-5 overflow-y-auto pr-1 max-h-[620px] scrollbar-thin scrollbar-thumb-purple-600/60 scrollbar-track-transparent">
            <h1 className="text-3xl font-extrabold tracking-tight text-white">{product.name}</h1>
            <p className="text-xl font-semibold text-purple-300">{product.price} VND</p>
            <p className="text-base text-gray-300 leading-relaxed whitespace-pre-line">{product.description}</p>
  
            <div className="flex items-center gap-3 mt-4">
              <span className="text-sm bg-pink-600/80 px-3 py-1 rounded-full shadow-md">
          ...
          <button
            ...
            className={`w-full py-3 mt-6 rounded-xl text-sm font-medium text-white transition-all duration-200 shadow-md ${
              isOutOfStock
